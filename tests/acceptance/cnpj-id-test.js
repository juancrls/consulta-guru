...
import { click, fillIn, visit, find, currentURL, waitUntil, settled, waitFor } from '@ember/test-helpers';
...
module('Acceptance | cnpj id', function (hooks) {
  ...
  test('should show loading div and data when accessed via URL', async function (assert) {
    ...
    assert.dom('[data-test-item-federal-tax-number]').hasText("00.000.000/0001-91", 'CNPJ data field should contain the correct data after URL change');
  });
  ...
  test('should show the correct data when user search multiple valid CNPJ using the input', async function (assert) {
    ...
    assert.strictEqual(currentURL(), '/consultar-cnpj-gratis/00000000000191', 'URL should have the correct CNPJ number after search');  
    assert.dom('[data-test-item-federal-tax-number]').hasText("00.000.000/0001-91", 'CNPJ data field should contain the correct data after search');

    await addInputAndSubmit('18792479000101');
    assert.dom('[data-test-item-federal-tax-number]').hasText("18.792.479/0001-01", 'CNPJ data field should contain the correct data after search');
    assert.strictEqual(currentURL(), '/consultar-cnpj-gratis/18792479000101', 'URL should have the correct CNPJ number after search');  
  });
  ...
  test('should keep the URL and add an error class to input if user tries to search for an invalid CNPJ using input', async function (assert) {
    ...
    assert.ok(find('[data-test-text-area]').classList.contains('error'), 'Text Area should have an error class');  
    
    assert.strictEqual(currentURL(), '/consultar-cnpj-gratis/00000000000191', 'URL should still be the same');  
    assert.dom('[data-test-item-federal-tax-number]').hasText("00.000.000/0001-91", 'CNPJ data field should still have the data for the URL CNPJ');
  });
  ...
  test('should display an "invalid cnpj" error message if user forces the URL to an invalid CNPJ', async function (assert) {
    ...
    assert.dom('[data-test-error-message]').hasText("CNPJ inválido inserido", "Invalid CNPJ message should be displayed")
    assert.ok(find('[data-test-text-area]').classList.contains('error'), 'Text Area should have an error class');  
  });
  ...
  test('should show the correct data when user search for an valid CNPJ after searching for a invalid', async function (assert) {
    ...
    await addInputAndSubmit('123');
    assert.strictEqual(currentURL(), '/consultar-cnpj-gratis/', 'URL should have the correct CNPJ number after search');  
    assert.notOk(find('[data-test-item-federal-tax-number]'), 'CNPJ data should NOT be displayed for incorrect CNPJ');  
  ...
  test('should show the correct data when user search for a different CNPJ than the one in the URL using the input', async function (assert) {
    ...
    await addInputAndSubmit('18792479000101');
    assert.strictEqual(currentURL(), '/consultar-cnpj-gratis/18792479000101', 'URL should have the correct CNPJ number before change');  
    assert.dom('[data-test-item-federal-tax-number]').hasText("18.792.479/0001-01", 'CNPJ data field should contain the correct data after search');
  });
  ...
  test('should show the correct data when user click on the back arrow in navigator (valid cnpj to valid cnpj)', async function (assert) {
    ...
    await waitUntil(() => find('[data-test-content-container]'));
    assert.dom('[data-test-item-federal-tax-number]').hasText("00.000.000/0001-91", 'CNPJ data field should contain the correct data before URL is changed');
    // AFTER URL CHANGED
    ...
    await waitUntil(() => find('[data-test-content-container]'));
    assert.dom('[data-test-item-federal-tax-number]').hasText("00.000.000/0001-91", 'CNPJ data field should contain the correct data after page back button is clicked'); // federal tax number should be equal to the URL number formatted
  });
